...
function Progresso() {
  ...
  this.atualizarPontos = (pontos) => {
    this.elemento.innerHTML = pontos; //colocando os pontos que receberam em parametross
  };
...
areaDoJogo.appendChild(new Progresso().elemnto);
...
function estaoSobrepostos(elementoA, elementoB) {
  ...
  const b = elementoB.getBoundingClientRect(); //retangulo relacionado ao ?
  ...
  this.start = () => {
    ...
    const temporizador = setInterval(() => {
      ...
      if (colidiu(passaro, barreiras)) {
        clearInterval(temporizador); // quando houver colisão
      }
    }, 20);
  };
}
